...
//!
//! Two kinds of consumer. An *interactive* one follows the fingers while they
//! are still down — that is what [`GestureUpdate`] is for. Everything else
//! waits for the release and resolves a [`SwipeGesture`] to an
//! [`Action`](crate::input::shortcuts::action::Action) through
//! `GestureBindings`, down the same path as a keyboard chord.
...
/// Release speed, in logical pixels per second, that fires a swipe on its own.
const COMMIT_VELOCITY: f64 = 500.0;
...
pub struct GestureUpdate {
    pub fingers: Fingers,
    pub axis: Axis,
    /// Cumulative travel along the locked axis, in logical pixels.
    pub delta: f64,
}
...
pub struct SwipeRelease {
    /// Logical pixels per second along the locked axis at the moment of
    /// release, signed like the travel. An interactive consumer hands this
    /// straight to its spring.
    pub velocity: f64,
...
impl GestureState {
    ...
    pub fn end(&mut self, cancelled: bool, at: Duration) -> Option<SwipeRelease> {
        ...
        let committed = !cancelled
            && (travelled.abs() >= COMMIT_DISTANCE || velocity.abs() >= COMMIT_VELOCITY);

        let gesture = axis.filter(|_| committed).map(|axis| {
            match (axis, travelled > 0.0) {
...
mod tests {
    ...
    fn axis_locks_only_after_the_threshold() {
        ...
        let update = state.update((20.0, 3.0), ms(20)).expect("locked by now");
    ...
    fn updates_report_cumulative_travel_along_the_locked_axis() {
        ...
        state.update((30.0, 0.0), ms(10));
        let update = state.update((18.0, 0.0), ms(20)).expect("active");
        assert_eq!(update.delta, 48.0);
    ...
    fn a_long_drag_commits() {
        ...
        let release = state.end(false, ms(30)).expect("a gesture was running");
        assert_eq!(
            release.gesture,
    ...
    fn a_short_fast_flick_commits_on_velocity_alone() {
        let mut state = started(4);
        // 40 px in 20 ms is 2000 px/s, well under the commit distance.
        state.update((20.0, 0.0), ms(10));
        state.update((20.0, 0.0), ms(20));
    ...
    fn a_second_gesture_does_not_inherit_the_first() {
        ...
        assert!(
            state.update((5.0, 0.0), ms(110)).is_none(),
