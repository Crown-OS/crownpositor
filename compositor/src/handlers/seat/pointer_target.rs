...
use smithay::{
    ...
    input::{
        keyboard::{KeysymHandle, LedState, ModifiersState},
...
/// Delegates to smithay's `impl PointerFocus for WlSurface`, which is what
...
impl PointerFocus<State> for PointerFocusTarget {
    fn enter(
        &self,
        seat: &Seat<State>,
        data: &mut State,
        ...
        if let Some(surface) = self.wl_surface() {
            PointerFocus::enter(surface.as_ref(), seat, data, keys, serial);
        }
    }

    fn leave(&self, seat: &Seat<State>, data: &mut State, serial: Serial) {
        if let Some(surface) = self.wl_surface() {
            PointerFocus::leave(surface.as_ref(), seat, data, serial);
        }
    }
    ...
    fn key(
        ...
        if let Some(surface) = self.wl_surface() {
            PointerFocus::key(surface.as_ref(), seat, data, key, state, serial, time);
        }
    }
    ...
    fn modifiers(
        ...
        if let Some(surface) = self.wl_surface() {
            PointerFocus::modifiers(surface.as_ref(), seat, data, modifiers, serial);
        }
    }
}
